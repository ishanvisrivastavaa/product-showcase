...
describe("AddToCartButton", () => {
  ...
  describe("compact mode", () => {
    it("labels the button with the product name for accessibility", () => {
      render(<AddToCartButton product={createMockProduct({ title: "Desk Lamp" })} compact />);

      expect(screen.getByRole("button", { name: "Add Desk Lamp to cart" })).toBeInTheDocument();
    });

    it("shows 'Sold out' and disables the button when there is no stock", () => {
      render(<AddToCartButton product={createMockProduct({ stock: 0 })} compact />);
    ...
    it("shows 'Add to cart' when the product is not yet in the cart", () => {
      render(<AddToCartButton product={createMockProduct({ id: 1 })} compact />);
    ...
    it("shows 'Added to cart' as soon as the product is already in the cart", () => {
      useCartStore.setState({
        items: [{ id: 1, title: "A", thumbnail: "", price: 10, stock: 5, quantity: 1 }],
      });
    ...
    it("still adds another unit when clicked while already in the cart", async () => {
      ...
      render(<AddToCartButton product={createMockProduct({ id: 1, stock: 5 })} compact />);
      await user.click(screen.getByRole("button"));
